...
int romanToDecimal(const char *roman) {
    ...
    for (int i = 0; roman[i] != '\0'; i++) {
        ...
        // Apply subtractive notation rule (e.g., IV = 4, IX = 9)
...
void decimalToRoman(int num, char *result) {
    ...
    // Append Roman symbols to result while subtracting values
...
void numberToWords(int num, char *result) {
    ...
    char temp[200] = "";    // Temporary storage for building the result
    int partCount = 0; 
    ...
    while (num > 0) {
        ...
        if (part > 0) {
            ...
            // Append place value (e.g., thousand, million)
            if (strlen(thousands[partCount]) > 0) {
                strcat(partResult, " ");
                strcat(partResult, thousands[partCount]);
            }

            // Combine the current part with the previous result
            strcat(partResult, " ");
...
void processFile(const char *input, const char *output) {
    ...
    while (fgets(line, sizeof(line), inputFile)) {
        ...
        // Parse the input line into operands and operator
